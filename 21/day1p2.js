...
const lines = readFileLines('data/day1.txt').map(Number)

let prevSum = lines.slice(0, 3).reduce((a, b) => a + b, 0)
let count = 0

for (let i = 3; i < lines.length; i++) {
    const num = lines[i]
    const thisSum = prevSum + num - lines[i - 3]
    if (thisSum > prevSum) count++
    prevSum = thisSum
}
