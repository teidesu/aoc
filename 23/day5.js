...
import { DefaultMap, IntRange, magic, readFileReader } from '../utils/index.js'
...
function doMap(source, target, value) {
    const map = parsedMaps.get(`${source}-to-${target}`)

    for (const { range, offset } of map) {
...
for (const seed of parsedSeeds) {
    const soil = doMap('seed', 'soil', seed)
    const fertilizer = doMap('soil', 'fertilizer', soil)
    const water = doMap('fertilizer', 'water', fertilizer)
    const light = doMap('water', 'light', water)
    const temperature = doMap('light', 'temperature', light)
    const humidity = doMap('temperature', 'humidity', temperature)
    const location = doMap('humidity', 'location', humidity)
