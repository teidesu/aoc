...
import { fp, magic, readFileReader, solveQuadraticInequality } from '../utils/index.js'

magic()

const lines = readFileReader('data/day6.txt')

const times = lines.at(0).skipUntil(/:\s+/).numbers()
const distances = lines.at(1).skipUntil(/:\s+/).numbers()

let sum = 1

for (const [time, distance] of fp.zip(times, distances)) {
    // hold * (time - hold) > distance
    // time * hold - hold^2 > distance
    // hold^2 - time * hold + distance < 0
    const solutions = solveQuadraticInequality(1, -time, distance, '<')
