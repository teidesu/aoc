...
found. In the example above, this is `2 + 1 + 0 + 1 + 2 + 5`, a total distance of `_11_`!

Your actual left and right lists contain many location IDs. _What is the total distance between your lists?_
*/

import { magic, readFileAsMatrix, transpose } from '../utils/index.js'
...
const left = cols[0].map(Number).sort((a, b) => a - b)
const right = cols[1].map(Number).sort((a, b) => a - b)
