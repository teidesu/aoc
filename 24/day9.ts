...
Another push of the button leaves you in the familiar hallways of some friendly [amphipods](/2021/day/23)! Good thing
you each somehow got your own personal mini submarine. The Historians jet away in search of the Chief, mostly by driving
directly into walls.
...
0`, `2 * 9 = 18`, `3 * 9 = 27`, `4 * 8 = 32`, and so on. In this example, the checksum is the sum of these, `_1928_`.

Compact the amphipod's hard drive using the process he requested. _What is the resulting filesystem checksum?_ (Be
...
for (let i = 0; i < line.length; i++) {
    ...
    for (let j = 0; j < char; j++) {
        ...
        if (isFreeSpace) {
            ...
            lastDataIdx = data.length - 1
        }
    }
}

while (gapsCount > 0) {
    const lastIdx = lastDataIdx--
    const firstGapIdx = data.indexOf(-1)

    data[firstGapIdx] = data[lastIdx]
    data[lastIdx] = -1
